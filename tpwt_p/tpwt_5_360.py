...
def process_in_per_dir(per, kern, smooth, damp, out_dir):
    """
    ...
    simanner_TPWT_run(kern, eq, bin_from=work)
...
def process_to_new_2d_of_per_dir(p, sec_snr_dis):
    # get eqlistper.v1.per
    find_bad_kern(p, bin_from=work)

    # get linenum of eqlistper.fine
    eq_fine = f'eqlistper.v1.{p.period}'
    content = f"{sec_snr_dis}\n"
    content += f"{p.snr} {p.dist} {p.nsta}\n"
    with open(eq_fine, 'a+') as f:
        f.write(content)

    shutil.copy(eq_fine, f"{p.output}/eqlistper{p.period}_{p.kern[0]}kern.v1")
...
def process_in_new_2d_of_per_dir(p, wave_type, sec_snr_dis):
    sensitivity_TPWT(p, wave_type, bin_from=work)

    # do rdsetupsimul phamp again
    per = p.period
    eqlistper = f'eqlistper{per}_{p.kern[0]}kern.v1'
    ...
    # simannerr 160 or 360 and create grid file
    simanner_and_gridgenvar(p, eqlistper, bin_from=work)
...
def process_period_grid(param, out_TPWT, wave_type):
    ...
    os.chdir(per_dir)
    param.vel = process_in_per_dir(per, param.kern[0], param.smooth, param.damp, param.output)

    sec_snr_dis = work / param.all_events / f"{per}sec_{param.snr}snr_{param.dist}dis"

    process_to_new_2d_of_per_dir(param, sec_snr_dis)

    # go into output directory, default is new_2d
    os.chdir(param.output)
    process_in_new_2d_of_per_dir(param, wave_type, sec_snr_dis)
...
def tpwt_05(param_json, *, snr=None, tcut=None, smooth=None, damping=None, kern=[300, 360]):
    global work
    work = pathlib.Path.cwd()

    # instancs a parameters class
    p5 = Param
    # get kern, [100, 160] or [300, 360]
    p5.kern = kern
    # get parameters
    ...
    p5.tevtrmscut = p_immu['tevtrmscut']
...
if __name__ == "__main__":
    tpwt_05("param.json", snr=10, tcut=6, smooth=50, damping=.2)
